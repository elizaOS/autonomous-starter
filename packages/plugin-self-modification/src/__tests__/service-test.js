...
const { CharacterModificationService } = require('../services/character-modification-service');
...
<character-modification>
  ...
  <reasoning>Rate limit test</reasoning>
    ...
    const passed = !result.success && result.errors?.includes('Modification rate limit exceeded');
...
<character-modification>
  ...
  <reasoning>Lock test</reasoning>
    ...
    const lockedPassed = !lockedResult.success && 
                         lockedResult.errors?.includes('Character modifications are currently locked');
    ...
    console.log('✅ Lock/unlock mechanism passed:', lockedPassed && unlockedPassed);
  } catch (error) {
  ...
  try {
    ...
    const passed = typeof initialVersion === 'number' && 
