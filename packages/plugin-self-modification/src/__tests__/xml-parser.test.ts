...
<character-modification>
  ...
  <reasoning>Multiple additions</reasoning>
    ...
    it('should throw error for missing root element', () => {
      ...
      expect(() => parseCharacterDiff(xml)).toThrow('Invalid XML: missing character-modification root element');
    });
...
<character-modification>
  <operations>
    <add path="bio[]" type="string">New bio</add>
      ...
      expect(() => parseCharacterDiff(xml)).toThrow('Missing or empty reasoning');
    });
...
<character-modification>
  ...
  <reasoning>Test</reasoning>
      ...
      expect(() => parseCharacterDiff(xml)).toThrow('Invalid operation type: invalidOp');
    });
...
<character-modification>
  ...
  <reasoning>Test</reasoning>
      ...
      expect(() => parseCharacterDiff(xml)).toThrow('Invalid path in add operation');
    });
...
<character-modification>
  ...
  <reasoning>Path traversal</reasoning>
        ...
        expect(() => parseCharacterDiff(dangerousPath1)).toThrow('Dangerous path pattern detected');
...
<character-modification>
  ...
  <reasoning>Double slash</reasoning>
        ...
        expect(() => parseCharacterDiff(dangerousPath2)).toThrow('Dangerous path pattern detected');
      });
    });
...
<character-modification>
  ...
  describe('buildCharacterDiffXml', () => {
    it('should build valid XML from diff object', () => {
      const diff: CharacterDiff = {
        operations: [
          ...
          { type: 'modify', path: 'system', value: 'Updated', dataType: 'string' },
      ...
      expect(xml).toContain('<modify path="system" type="string">Updated</modify>');
    ...
    it('should omit empty operation categories', () => {
      const diff: CharacterDiff = {
        operations: [
          { type: 'add', path: 'bio[]', value: 'New bio' }
        ],
    ...
    it('should validate reasoning is not empty', () => {
      ...
      expect(() => buildCharacterDiffXml(diff)).toThrow('Reasoning is required');
    });
    ...
    it('should validate operations is an array', () => {
      ...
      expect(() => buildCharacterDiffXml(diff)).toThrow('Operations must be an array');
    });
    
    it('should default dataType to string if not specified', () => {
      const diff: CharacterDiff = {
        operations: [
          { type: 'add', path: 'bio[]', value: 'No type' }
        ],
    ...
    it('should handle build errors gracefully', () => {
      ...
      expect(() => buildCharacterDiffXml(diff)).toThrow('Failed to build character diff XML');
    });
  });
    ...
    it('should maintain data integrity through parse and build', () => {
      const originalDiff: CharacterDiff = {
        operations: [
          ...
          { type: 'modify', path: 'adjectives[0]', value: 'creative', dataType: 'string' },
