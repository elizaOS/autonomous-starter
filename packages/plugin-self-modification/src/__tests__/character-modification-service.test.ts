...
<character-modification>
  ...
  <timestamp>2024-01-01T00:00:00Z</timestamp>
    ...
    it('should reject modifications when locked', async () => {
      ...
      expect(result.errors).toContain('Character modifications are currently locked');
    });
    ...
    it('should validate diff structure', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.[0]).toContain('Failed to parse character diff XML');
    });
    ...
    it('should handle persistence failure gracefully', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.[0]).toContain('Failed to update agent in database');
    });
  });
