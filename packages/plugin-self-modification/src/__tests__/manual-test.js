...
const { parseCharacterDiff, buildCharacterDiffXml } = require('../utils/xml-parser');
...
<character-modification>
  ...
  const passed = result.operations[0].value === '&xxe;' && !result.operations[0].value.includes('root:');
...
<character-modification>
  ...
  console.log('✅ Path traversal prevention passed:', error.message.includes('Dangerous path pattern'));
}
...
try {
  ...
  const passed = parsedDiff.operations.length === 3 && 
...
try {
  ...
  console.log('✅ Empty reasoning validation passed:', error.message.includes('Reasoning is required'));
}
