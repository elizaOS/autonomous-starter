...
export function applyOperationsToCharacter(
  ...
  for (const op of operations) {
    try {
      ...
      throw new Error(`Failed to apply operation ${op.type} at path ${op.path}: ${error.message}`);
    }
  }
...
function addValue(obj: any, path: string, value: any): void {
  ...
  if (path.includes('[]')) {
    ...
    const propertyName = normalizedPath.substring(normalizedPath.lastIndexOf('.') + 1);
    const parent = JSONPath({ path: parentPath, json: obj })[0];
...
export function validateCharacterStructure(character: any): boolean {
  ...
  const arrayFields = ['bio', 'lore', 'messageExamples', 'postExamples', 'topics', 'adjectives'];
