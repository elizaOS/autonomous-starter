...
export function validateCharacterDiff(diff: CharacterDiff): ValidationResult {
  ...
  for (const op of diff.operations) {
    ...
    if (op.path.includes('system') && op.type === 'modify') {
      if (typeof op.value === 'string' && op.value.length > MAX_SYSTEM_PROMPT_LENGTH) {
        errors.push(`System prompt too long (max ${MAX_SYSTEM_PROMPT_LENGTH} characters)`);
      }
    ...
    if (op.path.includes('[]') && op.type === 'add') {
      ...
      if (arrayPath.includes('messageExamples')) {
        warnings.push('Modifying message examples may affect agent behavior consistency');
      }
    }
    ...
    if (op.dataType) {
      if (!validateDataType(op.value, op.dataType)) {
        errors.push(`Invalid value type for ${op.path}: expected ${op.dataType}`);
      }
    }
  }
  ...
  const systemModifications = diff.operations.filter(op => 
    op.path.includes('system') && op.type === 'modify'
  );
  
  if (systemModifications.length > 0) {
    warnings.push('System prompt modifications can significantly alter agent behavior');
  }
...
export function validateModificationRate(
  ...
  return modificationsInLastHour < maxPerHour && modificationsInLastDay < maxPerDay;
}