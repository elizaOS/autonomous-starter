...
function isValidOperationType(type: string): type is 'add' | 'modify' | 'delete' {
...
export function parseCharacterDiff(xmlString: string): CharacterDiff {
  try {
    ...
    throw new Error(`Failed to parse character diff XML: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export function buildCharacterDiffXml(diff: CharacterDiff): string {
  ...
  try {
    ...
    throw new Error(`Failed to build character diff XML: ${error instanceof Error ? error.message : String(error)}`);
  }
}