...
export const modifyCharacterAction: Action = {
  ...
  description: 'Reflect on recent interactions and modify own character/personality',
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
    const modService = runtime.getService(CharacterModificationService.serviceName);
  ...
  handler: async (
    ...
    try {
      ...
      const xmlMatch = diffResponse.match(/<character-modification>[\s\S]*<\/character-modification>/);
      if (!xmlMatch) {
        callback?.({
