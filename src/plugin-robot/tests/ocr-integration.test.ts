...
describe('OCR Integration Tests with Real Images', () => {
  ...
  describe('Real Image Generation and OCR', () => {
    ...
    it('should generate images with special characters', async () => {
      const specialText = 'Email: test@example.com\nPhone: +1-555-123-4567\nPrice: $29.99';
    ...
    it('should handle different image dimensions', async () => {
      ...
      for (const dim of dimensions) {
        const imageBuffer = generateTextImage('Test Text', dim.width, dim.height);
        expect(imageBuffer.length).toBeGreaterThan(0);
      }
    });
  });
  ...
  describe('Performance with Real Images', () => {
    ...
    it('should handle multiple image processing requests', async () => {
      const images = Array(10).fill(0).map((_, i) => 
        generateTextImage(`Test ${i}`, 300, 100)
      );
...
function generateTextImage(text: string, width: number, height: number, fontSize: number = 24): Buffer {
...
function generateMultiLineTextImage(text: string, width: number, height: number, fontSize: number = 20): Buffer {
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext('2d');
