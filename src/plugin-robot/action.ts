...
export const performScreenAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      logger.info(`[performScreenAction] Executed ${executedActions} actions: ${actionSummary.join(', ')}`);
      ...
      logger.error('[performScreenAction] Error executing screen actions:', error);
      await callback({
  ...
  examples: [
    ...
    [
      { name: 'user', content: { text: 'fill in the email field with test@example.com' } },
      { name: 'agent', content: { actions: ['PERFORM_SCREEN_ACTION'] } },
    ],
  ],
};
