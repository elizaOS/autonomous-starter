...
import { RobotServiceType, type ScreenObject, type ScreenContext, type ScreenDescription, type RobotServiceConfig, type ChangeDetectionConfig } from './types';
...
export class RobotService extends Service {
  ...
  capabilityDescription = 'Controls the screen and provides recent screen context with intelligent change detection and local OCR.';
  ...
  private async captureScreen(): Promise<Buffer> {
    try {
      ...
      try {
        logger.debug(`[RobotService] Converting raw bitmap (${size.width}x${size.height}) to PNG format`);
        ...
        logger.debug(`[RobotService] Successfully converted to PNG (${pngBuffer.length} bytes)`);
        return pngBuffer;
      } catch (conversionError) {
        logger.error('[RobotService] Failed to convert bitmap to PNG:', conversionError);
  ...
  private downscaleImage(imageBuffer: Buffer, maxSize: number = 1024): Buffer {
    try {
      ...
      logger.warn('[RobotService] Image downscaling failed, using original:', error);
      return imageBuffer;
    }
  }
  ...
  private shouldTriggerAIProcessing(currentScreenshot: Buffer): { shouldTrigger: boolean; pixelDifference?: number } {
    if (!this.config.changeDetection.enabled || !this.previousScreenshot) {
      return { shouldTrigger: true };
    }

    const pixelDifference = this.calculatePixelDifference(currentScreenshot, this.previousScreenshot);
    const shouldTrigger = pixelDifference >= this.config.changeDetection.threshold;

    logger.debug(`[RobotService] Pixel difference: ${pixelDifference.toFixed(2)}%, Threshold: ${this.config.changeDetection.threshold}%, Trigger AI: ${shouldTrigger}`);
  ...
  private async performLocalOCR(image: Buffer): Promise<string> {
    if (!this.tesseractWorker) {
      logger.debug('[RobotService] Tesseract worker not available, falling back to AI OCR');
    ...
    try {
      ...
      logger.debug(`[RobotService] Performing local OCR with Tesseract.js on ${image.length} byte image`);
      ...
      if (!this.isValidImageBuffer(processedImage)) {
        logger.warn('[RobotService] Invalid image format, falling back to AI OCR');
        return this.performAIOCR(image);
      }
      
      const { data: { text } } = await this.tesseractWorker.recognize(processedImage);
      const duration = Date.now() - startTime;
      
      logger.debug(`[RobotService] Local OCR completed in ${duration}ms, extracted ${text.length} characters`);
      return text.trim();
    } catch (error) {
      logger.error('[RobotService] Local OCR failed, falling back to AI OCR:', error);
      return this.performAIOCR(image);
    }
  }
  ...
  private async performAIOCR(image: Buffer): Promise<string> {
    try {
      if (!image || image.length === 0) {
        logger.warn('[RobotService] performAIOCR received an empty image buffer.');
        return '';
      }

      const processedImage = this.downscaleImage(image, 1024);
      const imageUrl = `data:image/png;base64,${processedImage.toString('base64')}`;
      const ocrPrompt = 'Transcribe any text visible in this image. If no text is visible, respond with an empty string.';

      const result: any = await this.runtime.useModel(ModelType.IMAGE_DESCRIPTION, {
        imageUrl: imageUrl,
        prompt: ocrPrompt,
      });
      ...
      if (typeof result === 'string') {
        ...
        return result.description || result.text || result.title || JSON.stringify(result);
      }
      return '';
    } catch (error) {
  ...
  private async describeImage(image: Buffer): Promise<string> {
    try {
      ...
      const description = await this.runtime.useModel(ModelType.TEXT_SMALL, {
        prompt: 'Describe this screenshot in detail, focusing on the main elements, layout, and any significant changes.',
  ...
  private async detectObjects(image: Buffer): Promise<ScreenObject[]> {
    try {
      ...
      const result = await this.runtime.useModel(ModelType.OBJECT_SMALL, {
        prompt: 'Detect interactive objects with bounding boxes in this screenshot',
        image: processedImage,
      });
      ...
      logger.error('[RobotService] detectObjects error, defaulting to empty array:', e);
      return [];
    }
  }
  ...
  private updateDescriptionHistory(description: string, timestamp: number): void {
    ...
    if (this.descriptionHistory.length > this.config.maxHistoryEntries) {
      this.descriptionHistory = this.descriptionHistory.slice(0, this.config.maxHistoryEntries);
    }
  ...
  private async performUpdate(): Promise<void> {
    try {
      ...
      const { shouldTrigger, pixelDifference } = this.shouldTriggerAIProcessing(screenshot);
      ...
      if (shouldTrigger) {
        logger.debug('[RobotService] Significant change detected, running AI analysis...');
        ...
        logger.debug('[RobotService] No significant change detected, skipping AI analysis');
        // Use the most recent description from history
        description = this.descriptionHistory[0]?.description || 'No recent description available';
  ...
  async getContext(): Promise<ScreenContext> {
    ...
    if (!this.context || Date.now() - this.context.timestamp > this.config.cacheTTL) {
      // Start background update
      this.updateContextInBackground();
      
      // If we have stale context, return it immediately rather than blocking
      if (this.context) {
        logger.debug('[RobotService] Returning stale context while update runs in background');
        return this.context;
      }
      
      // Only block if we have no context at all (first run)
      logger.debug('[RobotService] No context available, waiting for initial update');
  ...
  setChangeDetectionThreshold(threshold: number): void {
    this.config.changeDetection.threshold = Math.max(0, Math.min(100, threshold));
    logger.info(`[RobotService] Change detection threshold set to ${this.config.changeDetection.threshold}%`);
  }

  enableChangeDetection(enabled: boolean): void {
    this.config.changeDetection.enabled = enabled;
    logger.info(`[RobotService] Change detection ${enabled ? 'enabled' : 'disabled'}`);
  }
}
