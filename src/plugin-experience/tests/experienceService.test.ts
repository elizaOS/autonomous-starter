...
describe('ExperienceService', () => {
  ...
  describe('recordExperience', () => {
    it('should record a basic experience', async () => {
      ...
      const experience = await experienceService.recordExperience(experienceData);
  ...
  describe('queryExperiences', () => {
    ...
    it('should query experiences by outcome', async () => {
      ...
      expect(experiences.every((e) => e.outcome === OutcomeType.POSITIVE)).toBe(true);
    });
    ...
    it('should filter by time range', async () => {
      ...
      expect(experiences.every((e) => e.createdAt >= oneHourAgo && e.createdAt <= now)).toBe(true);
    });
  ...
  describe('findSimilarExperiences', () => {
    ...
    it('should find similar experiences based on semantic similarity', async () => {
      const similar = await experienceService.findSimilarExperiences('shell command execution', 2);
    ...
    it('should handle embedding generation errors gracefully', async () => {
      ...
      const similar = await experienceService.findSimilarExperiences('test query', 5);
      expect(similar).toHaveLength(0);
    });
  });
  ...
  describe('analyzeExperiences', () => {
    ...
    it('should analyze experiences for a specific type', async () => {
      const analysis = await experienceService.analyzeExperiences('shell', ExperienceType.SUCCESS);
    ...
    it('should return empty analysis for no experiences', async () => {
      const analysis = await experienceService.analyzeExperiences('nonexistent');
    ...
    it('should generate recommendations based on reliability', async () => {
      ...
      expect(analysis.recommendations).toContain('Continue using successful approaches');
    });
  });
  ...
  describe('memory management', () => {
    it('should prune old experiences when limit is exceeded', async () => {
      ...
      const allExperiences = await experienceService.queryExperiences({ limit: 10 });
      expect(allExperiences.length).toBeLessThanOrEqual(3);

      // High importance experiences should remain
      const highImportanceRemaining = allExperiences.filter((e) => e.importance > 0.5);
  ...
  describe('error handling', () => {
    it('should handle embedding generation errors gracefully', async () => {
      mockRuntime.useModel = vi.fn().mockRejectedValue(new Error('Model error'));
    ...
    it('should handle event emission errors gracefully', async () => {
      mockRuntime.emitEvent = vi.fn().mockRejectedValue(new Error('Event error'));
