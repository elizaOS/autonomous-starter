...
describe('validation', () => {
    ...
    describe('validateEnvVar', () => {
        ...
        it('should handle validation errors gracefully', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith(
                'Error validating environment variable TEST_VAR:', new Error('Test error')
            );
        ...
        it('should use specific validation strategy when provided', async () => {
            ...
            const result = await validateEnvVar('OPENAI_API_KEY', 'sk-test123', 'api_key', 'api_key:openai');
            expect(result.isValid).toBe(true);
        ...
        describe('credential', () => {
            describe('database_url', () => {
                it('should return valid for proper database URL', async () => {
                    const result = await validationStrategies.credential.database_url('postgresql://user:pass@localhost:5432/db');
                    expect(result.isValid).toBe(true);
                    expect(result.details).toBe('Database URL format is valid');
                });

                it('should return valid for MongoDB URL', async () => {
                    const result = await validationStrategies.credential.database_url('mongodb://user:pass@localhost:27017/db');
                    expect(result.isValid).toBe(true);
                    expect(result.details).toBe('Database URL format is valid');
                });
