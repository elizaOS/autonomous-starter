...
import { IAgentRuntime, Character, Plugin, World, Service, UUID, logger } from '@elizaos/core';
...
describe('EnvManagerService', () => {
    ...
    describe('scanPluginRequirements', () => {
        it('should log a warning if WORLD_ID is not found', async () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith('[EnvManager] No WORLD_ID found, cannot scan plugin requirements');
        });
        ...
        it('should log a warning if world is not found', async () => {
            ...
            const warnSpy = vi.spyOn(logger, 'warn');
            await envService.scanPluginRequirements();
            expect(warnSpy).toHaveBeenCalledWith('[EnvManager] World not found, cannot scan plugin requirements');
        });
        ...
        it('should scan character secrets and loaded plugins', async () => {
            ...
            mockRuntime.character.settings = { secrets: { CHARACTER_API_KEY: 'test-key' } };
            mockRuntime.plugins = [
                {
                    name: 'test-plugin',
            ...
            expect(mockRuntime.updateWorld).toHaveBeenCalledWith(mockWorld);
            expect(logSpy).toHaveBeenCalledWith('[EnvManager] Plugin requirements scan completed');
        });
    ...
    describe('updateEnvVar', () => {
        it('should return false if WORLD_ID is not found', async () => {
            (mockRuntime.getSetting as any).mockReturnValue(null);
            const result = await envService.updateEnvVar('test-plugin', 'TEST_VAR', { value: 'test' });
        ...
        it('should not update process.env if updateEnvVar fails', async () => {
            ...
            (mockRuntime.getWorld as any).mockResolvedValue(mockWorld);
            (mockRuntime.updateWorld as any).mockRejectedValue(new Error('Update failed'));
    ...
    describe('getGeneratableEnvVars', () => {
        it('should return only missing env vars that can be generated', async () => {
            ...
            vi.spyOn(envService, 'getMissingEnvVars').mockResolvedValue(mockMissingVars);
