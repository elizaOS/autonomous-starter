...
function parseFileOperation(
  ...
  if (operationType && target) {
    ...
    const normalizedSecondaryTarget = secondaryTarget
      ...
      : undefined;
    return { operationType, target: normalizedTarget, secondaryTarget: normalizedSecondaryTarget };
  }
  return null;
}
...
export class ShellService extends Service {
  ...
  async executeCommand(
    command: string
  ): Promise<{ output: string; error?: string; exitCode: number | null; cwd: string }> {
