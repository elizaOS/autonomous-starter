...
export class PluginConfigurationService extends Service {
  ...
  async parsePluginRequirements(pluginPath: string): Promise<{
    ...
    try {
      ...
      logger.error(`[PluginConfigurationService] Failed to parse plugin requirements:`, error);
      return { requiredVars: [], optionalVars: [] };
    }
  }
  ...
  private async getOrCreateEncryptionKey(): Promise<Buffer> {
    ...
    try {
      ...
      logger.warn('[PluginConfigurationService] Failed to load encryption key, creating new one');
    }
  ...
  private async loadConfigurations(): Promise<void> {
    ...
    try {
      if (await fs.pathExists(configPath)) {
        ...
        logger.info(`[PluginConfigurationService] Loaded configurations for ${this.configStore.size} plugins`);
      }
    } catch (error) {
      logger.error('[PluginConfigurationService] Failed to load plugin configurations:', error);
    }
  }
  ...
  private async saveConfigurations(): Promise<void> {
    ...
    try {
      ...
      logger.error('[PluginConfigurationService] Failed to save plugin configurations:', error);
    }
  }
