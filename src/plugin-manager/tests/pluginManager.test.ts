...
const createMockRuntime = (): IAgentRuntime => {
  ...
  const runtimeMock = {
    ...
    registerEvaluator: vi.fn(async (evaluator: any) => { // any for mock evaluator
...
describe('PluginManagerService', () => {
  ...
  describe('Plugin Registration', () => {
    ...
    it('should throw error if plugin to load is not found', async () => {
      await expect(pluginManager.loadPlugin({ pluginId: 'non-existent-id' })).rejects.toThrow(
        'Plugin non-existent-id not found in registry'
      );
    });
    ...
    it('should throw error if plugin to load is not ready/unloaded and not forced', async () => {
      ...
      const pluginId = await pluginManager.registerPlugin(mockPlugin);
      pluginManager.updatePluginState(pluginId, { status: PluginStatus.BUILDING });
  ...
  describe('Plugin Unloading', () => {
    ...
    it('should throw error if plugin to unload is not found', async () => {
      await expect(pluginManager.unloadPlugin({ pluginId: 'non-existent-id' })).rejects.toThrow(
        'Plugin non-existent-id not found in registry'
      );
    });
    ...
    it('should handle errors during plugin component unregistration', async () => {
      const mockPluginWithService = createMockPluginWithService('service-unload-error');
      const pluginId = await pluginManager.registerPlugin(mockPluginWithService);
      await pluginManager.loadPlugin({ pluginId });

      // Simulate service stop error
      const serviceInstance = runtime.getService(MockService.serviceType);
      if (serviceInstance) {
        vi.spyOn(serviceInstance, 'stop').mockRejectedValueOnce(new Error('Service stop failed'));
      }

      await expect(pluginManager.unloadPlugin({ pluginId })).rejects.toThrow('Service stop failed');
      const pluginState = pluginManager.getPlugin(pluginId);
      expect(pluginState?.status).toBe(PluginStatus.ERROR);
    ...
    it('should correctly unregister components, including evaluators, and not remove original components', async () => {
      const originalAction: Action = { name: 'ORIGINAL_ACTION', handler: vi.fn(), similes: [], description: '', examples: [], validate: vi.fn() };
      const originalProvider: Provider = { name: 'ORIGINAL_PROVIDER', get: vi.fn(), description: '' };
      const originalEvaluator: Action = { name: 'ORIGINAL_EVALUATOR', handler: vi.fn(), similes: [], description: '', examples: [], validate: vi.fn() }; // Evaluator has Action shape for this mock
      class OriginalService extends Service { static override serviceType = 'ORIGINAL_SVC' as ServiceTypeName; override capabilityDescription = 'Original'; static async start() { return new this({} as any) } async stop() { } };
      ...
      runtime.services.set('ORIGINAL_SVC' as ServiceTypeName, new OriginalService({} as any));
      ...
      const mockPluginWithAll: Plugin = {
        ...
        evaluators: [{ name: 'TEST_EVALUATOR', handler: vi.fn(), similes: [], description: '', examples: [], validate: vi.fn() } as any], // cast as any
      ...
      if (mockPluginWithAll.services && mockPluginWithAll.services.length > 0) {
        ...
        if (serviceClass === MockService) { // ensure it's the one we expect
          runtime.services.set(MockService.serviceType, new MockService(runtime));
        }
      }

      expect(runtime.actions).toEqual(expect.arrayContaining([originalAction, mockPluginWithAll.actions![0]]));
      expect(runtime.providers).toEqual(expect.arrayContaining([originalProvider, mockPluginWithAll.providers![0]]));
      expect(runtime.evaluators).toEqual(expect.arrayContaining([originalEvaluator as any, mockPluginWithAll.evaluators![0]]));
      ...
      expect(runtime.services.has('ORIGINAL_SVC' as ServiceTypeName)).toBe(true); // Original service should remain
    });
  });
...
describe('Plugin Manager Actions', () => {
  ...
  describe('Load Plugin Action', () => {
    ...
    it('should handle plugin manager not available in handler', async () => {
      ...
      await loadPluginAction.handler(runtime, message, undefined, undefined, undefined);
  ...
  describe('Unload Plugin Action', () => {
    ...
    it('should handle plugin manager not available in handler', async () => {
      ...
      await unloadPluginAction.handler(runtime, message, undefined, undefined, undefined);
      // No assertion needed, just checking for no errors and coverage
    });
    ...
    it('should handle no exact match for plugin to unload in handler without callback', async () => {
      ...
      const loadedPlugin = pluginManager.getAllPlugins().find(p => p.name === 'loaded-plugin')!;
      await pluginManager.loadPlugin({ pluginId: loadedPlugin.id });
      const message = { content: { text: 'Unload non_existent_plugin' } } as any;
      await unloadPluginAction.handler(runtime, message, undefined, undefined, undefined);
      // No assertion needed, just checking for no errors and coverage
    });
    ...
    it('should handle plugin unloading error in handler without callback (not original plugin error)', async () => {
      ...
      vi.spyOn(pluginManager, 'unloadPlugin').mockRejectedValueOnce(new Error('Generic unload error'));
  ...
  it('should handle error during service registration in registerPluginComponents', async () => {
    class FailingService extends Service {
      static override serviceType: ServiceTypeName = 'FAILING_SERVICE' as ServiceTypeName;
    ...
    const pluginId = await pluginManager.registerPlugin(pluginWithFailingService);
    ...
    expect(runtime.getService('FAILING_SERVICE' as ServiceTypeName)).toBeUndefined();
  });
  ...
  it('should handle runtime.plugins being initially undefined during component registration', async () => {
    ...
    testRuntime.services.set('PLUGIN_MANAGER' as ServiceTypeName, pluginManager);
...
describe('Plugin State Provider', () => {
  ...
  it('should provide state for plugins with errors and building status', async () => {
    ...
    pluginManager.updatePluginState(errorPluginId, { status: PluginStatus.ERROR, error: 'Test Error' });
    ...
    pluginManager.updatePluginState(buildingPluginId, { status: PluginStatus.BUILDING });
    ...
    if (pluginState) { // Ensure pluginState is not null
  ...
  it('should provide a message when no plugins are registered', async () => {
    ...
    newRuntime.services.set('PLUGIN_MANAGER' as ServiceTypeName, new PluginManagerService(newRuntime));


    const result = await pluginStateProvider.get(newRuntime, {} as any, {} as any);
