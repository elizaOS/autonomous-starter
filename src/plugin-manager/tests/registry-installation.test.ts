...
import { type IAgentRuntime, type Plugin, Service, type ServiceTypeName, createUniqueUuid } from '@elizaos/core';
...
const createMockRuntime = (): IAgentRuntime => {
  ...
  const runtime = {
    ...
    evaluators: [],
    services,
    getService: vi.fn((serviceType: ServiceTypeName) => services.get(serviceType)),
...
describe('Registry Installation', () => {
  ...
  describe('installPluginFromRegistry', () => {
    ...
    it('should install plugin from npm repository', async () => {
      ...
      const pluginInfo = await pluginManager.installPluginFromRegistry(pluginName);
    ...
    it('should install plugin from git repository', async () => {
      ...
      expect(execa).toHaveBeenCalledWith(
        'git',
        ['clone', 'https://github.com/example/plugin-git-example.git', expect.any(String)],
        { stdio: 'pipe' }
      );
    });
    ...
    it('should throw error for non-existent plugin', async () => {
      ...
      await expect(pluginManager.installPluginFromRegistry(pluginName)).rejects.toThrow(
  ...
  describe('loadInstalledPlugin', () => {
    ...
    it('should throw error for non-installed plugin', async () => {
      ...
      await expect(pluginManager.loadInstalledPlugin(pluginName)).rejects.toThrow(
    ...
    it('should throw error for plugin requiring configuration', async () => {
      ...
      (fs.readJson as vi.Mock).mockImplementation(async (filePath) => {
        if (filePath.includes('package.json')) {
          return {
            ...
            version: '1.0.0',
            elizaos: {
              requiredEnvVars: [{ name: 'API_KEY', description: 'Required key', sensitive: true }]
            }
          };
        }
        throw new Error(`Mock not configured for ${filePath}`);
      ...
      await expect(pluginManager.loadInstalledPlugin(pluginName)).rejects.toThrow(
        'requires configuration before loading'
      );
    });
  });
  ...
  describe('listInstalledPlugins', () => {
    it('should return list of installed plugins', async () => {
      ...
      expect(installedPlugins.map(p => p.name)).toContain('@elizaos/plugin-example');
    });
